...
final readonly class ColumnPlan
{
     ...
     * @param bool        $transcode          rows with invalid UTF-8 exist and will be transcoded from the declared charset
     ...
     * ENUM/SET holds any non-ASCII value, because its declaration would have to change too. Otherwise:
     ...
     * with clean bytes.
     */
    public static function decide(Column $column, ColumnStats $stats, float $threshold, string $collation): self
    {
        $enumOrSet = in_array(Type::lookupName($column->getType()), [Types::ENUM, SetType::TYPE], true);
        $needsRedeclaration = $column->getCharset() !== self::TARGET_CHARSET || $column->getCollation() !== $collation;

        if (in_array($column->getCharset(), self::FIXED_WIDTH, true)) {
            return new self($column, $enumOrSet, $needsRedeclaration, false, false, false, 'fixed-width ' . $column->getCharset() . ' column, bytes cannot be classified');
        }
        if ($stats->invalid > 0 && ($enumOrSet || in_array($column->getCharset(), self::UTF8_FAMILY, true))) {
            return new self($column, $enumOrSet, $needsRedeclaration, false, false, false, 'invalid bytes in a ' . $column->getCharset() . ' column');
        }
        if ($enumOrSet && $stats->nonascii > 0) {
            return new self($column, $enumOrSet, $needsRedeclaration, false, false, false, 'non-ASCII enum/set');
        }

        $ratio = $stats->nonascii > 0 ? $stats->doubled / $stats->nonascii : 0.0;
        $undouble = $stats->doubled > 0 && $ratio >= $threshold;

        return new self($column, $enumOrSet, $needsRedeclaration, $stats->invalid > 0, $undouble, $stats->doubled > 0 && !$undouble, null);
    }
    ...
    /**
     * The plan as printed in the analysis table: `ok`, `redeclare [+transcode] [+undouble]` or
     * `manual (<reason>)`, with ` (review)` appended when double-encoded rows stay below the
     * threshold.
     */
    public function label(): string
    {
        ...
        $label = $this->needsRepair()
            ? 'redeclare' . ($this->transcode ? ' +transcode' : '') . ($this->undouble ? ' +undouble' : '')
            : 'ok';
